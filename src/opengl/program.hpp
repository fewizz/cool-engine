...
namespace gl {
	...
	public:
		...
		unsigned get_attrib_location(std::string attrib_name) {
			internal::get_attribute_location(name, attrib_name.c_str());
		}

		unsigned get_unifrom_location(std::string unifrom_name) {
			internal::get_uniform_location(name, unifrom_name.c_str());
		}
